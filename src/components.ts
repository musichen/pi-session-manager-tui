 ...
 * the color/weight helpers we actually call.
 */
...
export class SessionListView implements Component {
  ...
  render(width: number): string[] {
    ...
    lines.push(theme.fg("accent", theme.bold("pi session manager")));
    lines.push(
      theme.fg("dim", `search › ${this.query || "(type to fuzzy search)"}${this.query ? "▌" : ""}`),
    );
    lines.push(theme.fg("dim", "─".repeat(Math.max(1, width))));

    if (this.filtered.length === 0) {
      lines.push(theme.fg("warning", "  no sessions match your query"));
    } else {
      ...
      visible.forEach((session, i) => {
        ...
        const body = isSelected ? theme.bold(label) : label;
        const line = `${prefix} ${body}   ${theme.fg("dim", meta)}`;
        lines.push(truncateToWidth(line, width));
      });

      if (this.filtered.length > this.maxVisible) {
        const remaining = this.filtered.length - this.maxVisible;
        lines.push(theme.fg("dim", `  … ${remaining} more (scroll or refine search)`));
      }
    }

    lines.push(theme.fg("dim", "─".repeat(Math.max(1, width))));
    lines.push(theme.fg("dim", "↑↓ navigate · type to search · enter open · esc back/exit"));
...
// ---------------------------------------------------------------------------
// History view: scrollable transcript
// ---------------------------------------------------------------------------
...
export class HistoryView implements Component {
  ...
  private rolePrefix(role: string, theme: ThemeLike): string {
    switch (role) {
      ...
      case "assistant":
        return theme.fg("muted", theme.bold("agent"));
      case "tool":
  ...
  private buildLines(width: number): string[] {
    ...
    for (const item of transcript) {
      ...
      const prefixWidth = item.role === "user" ? 3 : 5;
      const wrapWidth = Math.max(6, textWidth - prefixWidth);
  ...
  render(width: number): string[] {
    ...
    const header = theme.fg("accent", theme.bold("history"));
    const position = allLines.length === 0 ? "empty" : `line ${this.offset + 1}/${allLines.length}`;
    const header2 = theme.fg("dim", `${position} · ↑↓ scroll · PgUp/PgDn jump · esc/← back`);

    const window = allLines.slice(this.offset, this.offset + this.maxHeight);

    const lines = [header, header2, theme.fg("dim", "─".repeat(Math.max(1, width))), ...window];
    if (allLines.length > this.maxHeight) {
      lines.push(theme.fg("dim", `… ${maxOffset} more lines below`));
    }
